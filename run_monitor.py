...
def main():
    ...
    while True:
        ...
        if new_stargazers:
            ...
            send_notification(message, slack_icon_emoji=':star:')
        else: 
        ...
        if new_repo_forks:
            ...
            send_notification(message, slack_icon_emoji=':fork_and_knife:')
        else:
        ...
        if new_repo_issues:
            ...
            send_notification(message, slack_icon_emoji=':chart_with_upwards_trend:')
        else:
        ...
        if new_org_followers:
            message = f"🙋 {len(new_org_followers)} new org followers for {owner}\n\ntotal org followers : {total_org_followers}"
            send_notification(message, slack_icon_emoji=':busts_in_silhouette:')
        else:
        ...
        if int(total_new_downloads[0]['new_downloads']) > 0:
            ...
            for asset in asset_downloads:
                if int(asset['new_downloads']) > 0:
                    send_notification(f"📦 new downloads for {asset['asset_name']}: {asset['new_downloads']}", slack_icon_emoji=':package:')
        # refresh state
        ...
        if int(new_downloads[0]['new_downloads']) > 0:
                ...
                send_notification(message, slack_icon_emoji=':beer:')
        else:
